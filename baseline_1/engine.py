...
def evaluate(model: torch.nn.Module, original_model: torch.nn.Module, data_loader, 
    ...
    plt.savefig(f'icmem_output/prompt_pool_{curr_task}_{task_id+1}.png')
    plt.close()
...
def evaluate_till_now(model: torch.nn.Module, original_model: torch.nn.Module, data_loader, 
    ...
    print(f"Standard Deviation of Similarities for task {task_id+1}: {std_dev:.4f}")
...
# initialize prompt pool and prompt key for each task to previous task's prompt pool and prompt key 
# Call other methods to train and evaluate the model
