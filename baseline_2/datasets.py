...
def get_dataset(dataset, transform_train, transform_val, args,):
    if dataset == 'CIFAR100':
        ...
        dataset_train = ICMemDataset(args.data_path, train=True, transform=transform_train, baseline_file_high=args.baseline_file_high, baseline_file_low=args.baseline_file_low)
        dataset_val = ICMemDataset(args.data_path, train=False, transform=transform_val, baseline_file_high=args.baseline_file_high, baseline_file_low=args.baseline_file_low)
    else:
...
def split_single_dataset(dataset_train, dataset_val, args):
    ...
    for num_classes in class_splits:
        ...
        if args.dataset == "Split-ICMem" and num_classes == 1:
        ...
        # Each time we call random its state is updated so mixed has diff task 1 since it calls random twice more
        if args.replay_size > 0 and args.dataset == "Split-ICMem":
            if num_classes == 2: 
                hm_images_class1 = [idx for idx in train_split_indices if dataset_train.mem[idx] == 1 and dataset_train.targets[idx] == scope[0]]
                hm_images_class2 = [idx for idx in train_split_indices if dataset_train.mem[idx] == 1 and dataset_train.targets[idx] == scope[1]]
                lm_images_class1 = [idx for idx in train_split_indices if dataset_train.mem[idx] == 0 and dataset_train.targets[idx] == scope[0]]
                lm_images_class2 = [idx for idx in train_split_indices if dataset_train.mem[idx] == 0 and dataset_train.targets[idx] == scope[1]]
                random.shuffle(train_split_indices)
                random.shuffle(test_split_indices)

                if args.baseline_mode == "high":
                    replay_buffer[scope[0]] = random.sample(hm_images_class1, args.replay_size)
                    replay_buffer[scope[1]] = random.sample(hm_images_class2, args.replay_size)
                elif args.baseline_mode == "low":
                    replay_buffer[scope[0]] = random.sample(lm_images_class1, args.replay_size)
                    replay_buffer[scope[1]] = random.sample(lm_images_class2, args.replay_size)
                elif args.baseline_mode == "mixed":
                    replay_buffer[scope[0]] = random.sample(hm_images_class1, args.replay_size // 2) + random.sample(lm_images_class1, args.replay_size // 2)
                    replay_buffer[scope[1]] = random.sample(hm_images_class2, args.replay_size // 2) + random.sample(lm_images_class2, args.replay_size // 2)
                
            else:
                hm_images = [idx for idx in train_split_indices if dataset_train.mem[idx] == 1]
                lm_images = [idx for idx in train_split_indices if dataset_train.mem[idx] == 0]
                
                if args.baseline_mode == "high":
                    replay_buffer[scope[0]] = random.sample(hm_images, args.replay_size)

                elif args.baseline_mode == "low":
                    replay_buffer[scope[0]] = random.sample(lm_images, args.replay_size)

                elif args.baseline_mode == "mixed":
                    replay_buffer[scope[0]] = random.sample(hm_images, args.replay_size // 2) + random.sample(lm_images, args.replay_size // 2)

                # Collect all replay images from the buffer
                for past_class, indices in replay_buffer.items():
                    if past_class not in scope:
                        train_split_indices.extend(indices)
